...
SIGNAL_RET_ONE_PARAM(MyDelegate, int, double);
MyDelegate d1, d2, d3, d4, d5, d6, d7, d8, d9;

SIGNAL_RET_ONE_PARAM(MyMultiDelegate, int, double);
MyMultiDelegate md1;
...
int main(int argc, char **argv)
{
    ...
    d1.Bind<MyClass, &MyClass::MemberFunction>(mc);
    d1(11.0);

    d2.Bind<MyClass, &MyClass::ConstMemberFunction>(mc);
    d2(1.0);
    ...
    d3.Bind<const MyClass, &MyClass::ConstMemberFunction>(cmc);
    d3(1.1);
    ...
    md1.Bind<MyClass, &MyClass::MemberFunction>(mc);
    md1.Bind<MyClass, &MyClass::ConstMemberFunction>(mc);
    md1.Bind<const MyClass, &MyClass::ConstMemberFunction>(cmc);
